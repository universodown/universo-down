...
$(document).ready(function () {
  ...
  $("#formUpdate").submit(function (e) {
    ...
    if (checkProperties(data) === false) {
      try {
        $.ajax({
          url: baseUrlApi + "/transport-request/" + id.value,
          type: "PUT",
          headers: {
            "x-access-token": localStorage.getItem("Authorization"),
          },
          ...
          success: function (data) {
            if (data != "") {
              ...
              window.location.href =
                "/frontend/html/listar/transportRequest.html";
            }
          },
          error: function (err) {
            erroHandler(err.status);
...
export function loadInfoDataTable() {
  $(document).ready(function () {
    $.ajax({
      type: "GET",
      url: baseUrlApi + "/transport-request/assisted/",
      headers: {
        "x-access-token": localStorage.getItem("Authorization"),
      },
      dataType: "JSON",
      success: function (data) {
        ...
        $("#transportRequestTable").DataTable({
          language: {
    ...
    $("#transportRequestTable").on("click", ".btn-primary", function () {
      var id = $(this).attr("id").match(/\d+/)[0];
      var data = $("#transportRequestTable").DataTable().row(id).data();
      window.location.href = "../editar/transportRequest.html?id=" + data.id;
    });
  });
}
...
export function loadInfoById() {
  $(document).ready(function () {
    ...
    if (params.has("id")) {
      ...
      $.ajax({
        type: "GET",
        url: baseUrlApi + "/transport-request/" + id,
        headers: {
          "x-access-token": localStorage.getItem("Authorization"),
        },
        dataType: "JSON",
